...
rm(list = ls())
setwd("C:/Users/sudhi/Desktop/PhD projects/NS-Estimator/Replication of Coordinatewise Gaussianization/Results Aug 31, 2026")
library(Tlasso)
library(tensr)
library(glasso)
library(expm)
...
source("C:/Users/sudhi/Desktop/PhD projects/NS-Estimator/simulation.summary.R")
source("C:/Users/sudhi/Desktop/PhD projects/NS-Estimator/Lfunctions.R")
source("C:/Users/sudhi/Desktop/PhD projects/NS-Estimator/Separate.fit.correct.R")
source("C:/Users/sudhi/Desktop/PhD projects/NS-Estimator/Separate.fit.correct.plot.R")
source("Model.R")
...
for(itr in 1:RUNs){
  ...
  x.unchanged <- data[[1]]$x
  vax.unchanged <- data[[1]]$vax
  
  x <- array(NA, dim=c(dimen, n))
  vax <- array(NA, dim=c(dimen, n))
  
  for(i in 1:50){
    x[, , i] <- x.unchanged[i, ]
    vax[, , i] <- vax.unchanged[i, ]
  }
  Sigma <- data[[2]]
  Omega <- data[[3]]
  
  nOut <- ceiling(pctOut * n)
  idxcontami.x <- sample(n, nOut)
  contami <- matrix(rnorm(nOut * dimen[[1]], 10, sqrt(0.2)), c(dimen[[1]], nOut))
  dim(contami) <- c(dimen, nOut)
  
  contam.x <- x[]
  for(i in seq_along(idxcontami.x)){
    contam.x[, , idxcontami.x[i]] <- contami[, , i]
  }
  
  contam.Tx <- NSEstimator2(contam.x, dimen, n)
  
  
  idxcontami.vax <- sample(n, nOut)
  contami <- matrix(rnorm(nOut * dimen[[1]], 10, sqrt(0.2)), c(dimen[[1]], nOut))
  dim(contami) <- c(dimen, nOut)
  
  contam.vax <- vax[]
  for(i in seq_along(idxcontami.vax)){
    contam.vax[, , idxcontami.vax[i]] <- contami[, , i]
  }
  
  contam.Tvax <- NSEstimator2(contam.vax, dimen, n)
  
  # Tx <- NSEstimator(x, dimen)
  # Tvax <- NSEstimator(vax, dimen)
  Tx <- NSEstimator2(x, dimen, n)
  Tvax <- NSEstimator2(vax, dimen, n)
  
  if(itr %in% x.values){
    png(paste0("Aug31.2026.1.x.Plot", itr, ".png"), width = 800, height = 600)
  ...
  if(itr %in% contam.x.values){
    png(paste0("Aug31.2026.1.contam.Plot", itr, ".png"), width = 800, height = 600)
  ...
  if(itr %in% contam.Tx.values){
    png(paste0("Aug31.2026.1.contam.Tx.Plot", itr, ".png"), width = 800, height = 600)
  ...
  if(itr %in% contam.Tx.values.C){
    png(paste0("Aug31.2026.1.contam.Tx.C.Plot", itr, ".png"), width = 800, height = 600)
  ...
  outx <- simulation.summary(list(fitx$Omegahat[[1]][[1]]), list(Omega), offdiag = FALSE)
  outx.sigma <- simulation.summary(list(fitx$Omegahat[[1]][[2]]), list(Sigma), offdiag = FALSE)
  ...
  contam.outx <- simulation.summary(list(contam.fitx$Omegahat[[1]][[1]]), list(Omega), offdiag = FALSE)
  contam.outx.sigma <- simulation.summary(list(contam.fitx$Omegahat[[1]][[2]]), list(Sigma), offdiag = FALSE)
  ...
  contam.outTx <- simulation.summary(list(contam.fitTx$Omegahat[[1]][[1]]), list(Omega), offdiag = FALSE)
  contam.outTx.sigma <- simulation.summary(list(contam.fitTx$Omegahat[[1]][[2]]), list(Sigma), offdiag = FALSE)
  ...
  contam.outTx.C <- simulation.summary(list(contam.fitTx.C$Omegahat[[1]][[1]]), list(Omega), offdiag = FALSE)
  contam.outTx.C.sigma <- simulation.summary(list(contam.fitTx.C$Omegahat[[1]][[2]]), list(Sigma), offdiag = FALSE)
...
cat("Frob. Norm True Sigma:", norm(Sigma, type="F"), "\n")
cat("Mean Frob. Norm Estimated Omega:", colMeans(est.omega), "SD:", sd(est.omega), "\n")
cat("Frob. Norm True Omega:", norm(Omega, type="F"), "\n")
